import numpy as np
import pandas as pd
from src.gauss_bayes import GaussianNaiveBayes
...
feature_names = ['weight_kg', 'body_length_cm', 'tail_length_cm', 'arm_span_cm', 'canine_size_mm']
...
print(f"Class Priors (P(class)): {classifier.class_priors}")
print(f"Means (per class, per feature):\n{classifier.class_means}")
print(f"Variances (per class, per feature):\n{classifier.class_vars}")
print("\n")
...
for i in range(min(5, len(X_test))):
    print(f"Actual: {y_test[i]}, Predicted: {y_pred[i]}, Probabilities: {np.round(y_pred_proba[i], 3)}")
print("\n")
